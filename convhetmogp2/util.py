...
#Modifications by Juan Jose Giraldo from University of Sheffield (2020):
#The have been included new functions to allow convolution processes
...
def conv_cross_covariance_full(X, Z, B, kernel_list, kernel_list_Gdj, kfu_aux,d):
    """
    Builds the cross-covariance cov[f_d(x),u(z)] of a Convolved Multi-output GP
    :param X: Input data
    :param Z: Inducing Points
    ...
    Kfdu = np.zeros([N,M])
    for q, B_q in enumerate(B):
        update_conv_Kff(kernel_list[q], kernel_list_Gdj[d], kfu_aux)
        Kfdu += B_q.B[d,d]*kfu_aux.K(X,Z[:, d*Xdim:d*Xdim+Xdim]) #+ 1e-2*np.eye(X.shape[0])) #verificar si se actualiza B

    return Kfdu

def both_convoled_Kff_and_Kfu_full(X, Z, B, kernel_list, kernel_list_Gdj, k_aux,d):
    """
    Builds the cross-covariance cov[f_d(x),u(z)] of a Convolved Multi-output GP
    :param X: Input data
    :param Z: Inducing Points
    ...
    Kfdfd = np.zeros((N, N))
    for q, B_q in enumerate(B):
        update_conv_Kff(kernel_list[q], kernel_list_Gdj[d], k_aux)
        Kfdfd += B_q.B[d, d] * k_aux.K(X, X)
        Kfdu += B_q.B[d,d]*k_aux.K(X,Z[:, d*Xdim:d*Xdim+Xdim]) #+ 1e-2*np.eye(X.shape[0])) #verificar si se actualiza B
...
def latent_function_covKuu(Z, B, kernel_list, kernel_list_Gdj, kff_aux):
    ...
    for j in range(J):
        for q, B_q in enumerate(B):
            ...
            Kuu[j, :, :] += B_q.B[j, j] * kff_aux.K(Z[:, j * Xdim:j * Xdim + Xdim], Z[:, j * Xdim:j * Xdim + Xdim])
...
def update_gradients_full(coreg, dL_dK, X, X2=None):
    ...
    dW = 2.0*(coreg.W[:, None, :]*dL_dK_small[:, :, None]).sum(0)
